...
{
    // return if URI is not valid or is no fleacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("fleacoin"))
        return false;
...
{
    // Convert fleacoin:// to fleacoin:
    //
    //    Cannot handle this later, because fleacoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("fleacoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "fleacoin:");
    }
...
{
    QString ret = QString("fleacoin:%1").arg(info.address);
    int paramCount = 0;
...
{
    return GetAutostartDir() / "fleacoin.desktop";
}
